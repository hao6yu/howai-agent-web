-- Master AI Personality Template System for HaoGPT Web
-- This schema adds a master/default personality that all user personalities are based on

-- Drop existing constraints if needed to modify the table
ALTER TABLE IF EXISTS ai_personalities DROP CONSTRAINT IF EXISTS ai_personalities_user_id_fkey;

-- Modify ai_personalities table to support both master and user personalities
ALTER TABLE ai_personalities
  ALTER COLUMN user_id DROP NOT NULL,
  ADD COLUMN IF NOT EXISTS is_master BOOLEAN DEFAULT false,
  ADD CONSTRAINT unique_master UNIQUE (is_master) WHERE is_master = true,
...
ALTER TABLE ai_personalities
  ADD CONSTRAINT ai_personalities_user_id_fkey
  FOREIGN KEY (user_id)
...
) ON CONFLICT (is_master) WHERE is_master = true
DO UPDATE SET
  updated_at = NOW();
...
-- Only admins can modify master (you can adjust this based on your admin system)
-- For now, no one can modify master through the application
CREATE POLICY "No one can modify master personality" ON ai_personalities
...
SELECT
  ...
  CASE
    ...
    WHEN ap.updated_at > m.updated_at THEN 'Customized'
    ELSE 'Using defaults'
  END as status
FROM profiles p
